...
vols, energies = np.loadtxt('${seed}-${magst}_EV.dat',unpack=True)
...
print(" Results for magnetic state=${magst}")
...
#plt.xlim(15, 34)
#plt.ylim(-4930.5, -4426.6)

x = np.linspace(min(vols), max(vols), 50)
y = Murnaghan(params, x)
plt.plot(x, y, c=col, label='magnetic state=${magst}')
